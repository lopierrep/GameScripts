...
CATEGORY_WORKERS = 8  # llamadas paralelas al endpoint /items


def _fetch_one_category(item_id: int) -> tuple[int, str]:
    try:
        resp = requests.get(f"{BASE_URL}/items/{item_id}", timeout=10)
        if resp.status_code == 404:
            return item_id, ""
        resp.raise_for_status()
        type_obj = resp.json().get("type", {})
        name_obj = type_obj.get("name", {})
        return item_id, name_obj.get("es", name_obj.get("en", ""))
    except Exception as e:
        print(f"    [WARN] No se pudo obtener categoría para item {item_id}: {e}")
        return item_id, ""


def fetch_categories(recipes: list[dict]) -> dict[int, str]:
    """Consulta categorías en paralelo y devuelve {id: categoría}."""
    ids = [r["id"] for r in recipes]
    categories: dict[int, str] = {}
    with ThreadPoolExecutor(max_workers=CATEGORY_WORKERS) as pool:
        futures = {pool.submit(_fetch_one_category, item_id): item_id for item_id in ids}
        for future in as_completed(futures):
            item_id, cat = future.result()
...
def save(job_name: str, recipes: list[dict]):
    ...
    need_category: list[dict] = []
    for r in recipes:
        rec = parse_recipe(r)
        old = existing.get(rec["id"])

        if old is None:
            # Receta nueva: añadir tal cual (categoría pendiente)
            parsed.append(rec)
            need_category.append(rec)
            added += 1
        elif _ingredients_changed(old.get("ingredients", []), rec["ingredients"]):
            # Ingredientes cambiaron: actualizar ingredientes, conservar categoría existente
            rec["category"] = old.get("category", "")
            if not rec["category"]:
                need_category.append(rec)
            parsed.append(rec)
            changed += 1
        else:
            # Sin cambios: conservar todo, pero consultar categoría si falta
            if not old.get("category"):
                need_category.append(old)
            parsed.append(old)
            skipped += 1

    if need_category:
        print(f"  -> Consultando categoría para {len(need_category)} recetas nuevas…")
        cats = fetch_categories(need_category)
        for rec in need_category:
            rec["category"] = cats.get(rec["id"], "")
