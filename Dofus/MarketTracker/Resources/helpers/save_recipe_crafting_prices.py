...
def load_all_pack_prices() -> dict[str, dict]:
    ...
    pack_prices = {}
    lots = {"x1": 1, "x10": 10, "x100": 100, "x1000": 1000}

    # Precios de recursos y otros ingredientes
    for data in (_load_file(srs.ITEMS_FILE), _load_file(OTHER_JSON)):
        for items in data.values():
            for item in items:
                name = item["name"].strip()
                pack_prices[name] = {
                    size: (item[f"unit_price_{size}"] / qty if item.get(f"unit_price_{size}", 0) > 0 else 0)
                    for size, qty in lots.items()
                }
