...
def ocr_all_prices() -> tuple[dict, "Image.Image", str]:
    ...
    return prices, processed, raw


def _save_debug(name: str, attempt: int, img: "Image.Image", raw: str):
    import datetime
    debug_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "..", "Debug")
    os.makedirs(debug_dir, exist_ok=True)
    ts = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
    safe_name = re.sub(r"[^\w]", "_", name)
    base = os.path.join(debug_dir, f"{safe_name}_{ts}_intento{attempt}")
    img.save(base + ".png")
    with open(base + ".txt", "w", encoding="utf-8") as f:
        f.write(raw)
    print(f"  [debug] guardado en {base}.*")
